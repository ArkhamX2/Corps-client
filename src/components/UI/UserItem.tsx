...
const UserItem : FC<UserItemProps> = (props: UserItemProps) => {
    return (
        ...
        <div style={{display: 'flex', flexDirection:'row' ,alignItems: 'center', justifyContent: 'flex-start', ...props.style}}>
            <img style={{ width: '100px', height: '100px' }} src={`data:image/png;base64, ${props.userData.find(x => x.id === props.item.avatarId)!.imageData}`} alt={`UserIcon ${props.item.avatarId}`} />
